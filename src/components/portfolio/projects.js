...
var portfolio_data_1 = require("@/lib/portfolio-data");
/* ============================================================
   ...
   ============================================================ */
function Projects() {
    var categories = ["All", "Data & ML", "Backend & DB", "Full-Stack", "Web3"];
    ...
    var reduced = (0, react_1.useReducedMotion)();
    var filteredProjects = React.useMemo(function () {
        return selectedCategory === "All"
            ? portfolio_data_1.projects
            : portfolio_data_1.projects.filter(function (project) { return project.tags.includes(selectedCategory); });
    }, [selectedCategory]);
    ...
    var onScroll = React.useCallback(function () {
        ...
        if (!el)
            return;
        var count = Math.max(filteredProjects.length, 1);
        var cardWidth = el.scrollWidth / count;
        var idx = Math.round(el.scrollLeft / cardWidth);
    ...
    var onMouseMove = React.useCallback(function (e) {
        if (!dragStart.current || !trackRef.current)
            return;
    ...
    var endDrag = React.useCallback(function () {
        ...
        setTimeout(function () { return onScroll(); }, 60);
    }, [onScroll]);
    return (<section id="work" className="relative scroll-mt-20 py-24 md:py-36 border-t border-rule overflow-hidden" aria-labelledby="work-heading">
      {/* ── Section header ── */}
      <div className="container-editorial mb-10 md:mb-14">
        <primitives_1.Reveal>
          <primitives_1.SectionLabel index="04" eyebrow="Selected Work" title={<>
                Engineering, not{" "}
                <span className="italic font-normal text-ink-soft">screenshots</span>.
              </>}/>
        </primitives_1.Reveal>

        <primitives_1.Reveal delay={0.1}>
          <div className="mt-8 flex flex-col gap-6">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
              ...
              <div className="flex items-center gap-3 shrink-0">
                <button aria-label="Previous project" onClick={function () { return scrollTo(Math.max(0, activeIndex - 1)); }} disabled={activeIndex === 0} className="group h-10 w-10 rounded-full border border-rule-strong flex items-center justify-center transition-all duration-200 hover:border-ink hover:bg-ink hover:text-paper disabled:opacity-30 disabled:pointer-events-none">
                ...
                <button aria-label="Next project" onClick={function () { return scrollTo(Math.min(filteredProjects.length - 1, activeIndex + 1)); }} disabled={activeIndex === filteredProjects.length - 1} className="group h-10 w-10 rounded-full border border-rule-strong flex items-center justify-center transition-all duration-200 hover:border-ink hover:bg-ink hover:text-paper disabled:opacity-30 disabled:pointer-events-none">
                  <svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden>
            ...
            <div className="flex flex-wrap items-center gap-3">
              {categories.map(function (category) { return (<button key={category} type="button" onClick={function () {
                setSelectedCategory(category);
                setActiveIndex(0);
            }} className={"rounded-full border px-4 py-2 text-sm font-medium transition ".concat(selectedCategory === category
      ...
      {/* ── Carousel track ── */}
      <div ref={trackRef} onScroll={onScroll} onMouseDown={onMouseDown} onMouseMove={isDragging ? onMouseMove : undefined} onMouseUp={endDrag} onMouseLeave={endDrag} style={{
            cursor: isDragging ? "grabbing" : "grab",
            paddingLeft: "clamp(1.25rem, 4vw, 3rem)",
            paddingRight: "clamp(1.25rem, 4vw, 3rem)",
        }} className="flex gap-5 overflow-x-auto snap-x snap-mandatory scrollbar-none pb-4 select-none">
        <react_1.AnimatePresence initial={false} mode="popLayout">
          {filteredProjects.map(function (project, i) { return (<ProjectCard key={project.id} project={project} index={i + 1} isActive={i === activeIndex} onClick={function () { return scrollTo(i); }}/>); })}
      ...
      <div className="container-editorial mt-8 flex items-center gap-2">
        {filteredProjects.map(function (_, i) { return (<button key={i} aria-label={"Go to project ".concat(i + 1)} onClick={function () { return scrollTo(i); }} className={[
...
function ProjectCard(_a) {
    var project = _a.project, index = _a.index, isActive = _a.isActive, onClick = _a.onClick;
    var metric = extractMetric(project);
    return (<dialog_1.Dialog onOpenChange={function (open) {
            if (open)
                onClick();
        }}>
      <dialog_1.DialogTrigger asChild>
        <react_1.motion.article layout initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -18 }} transition={{ duration: 0.32, ease: "easeOut" }} className={[
      ...
      <div className="flex flex-col flex-1 px-5 pt-4 pb-5 bg-paper">
        ...
        {metric && metric.value !== "•" ? (<div className="flex items-baseline gap-2">
          ...
          </div>) : (<p className="text-[12px] text-ink-soft leading-snug line-clamp-2">
            {project.outcome}
          </p>)}

        {/* Row 5: stack pills — push to bottom */}
        <div className="mt-auto pt-3.5 flex flex-wrap gap-1">
          {project.stack.slice(0, 3).map(function (s) { return (<span key={s} className="rounded-sm border border-rule px-1.5 py-0.5 font-mono-label text-[8px] uppercase tracking-[0.1em] text-ink-soft/90">
              {s}
            </span>); })}
          {project.stack.length > 3 && (<span className="font-mono-label text-[8px] text-ink-mute/50 self-center pl-0.5">
              +{project.stack.length - 3}
            </span>)}
        </div>

        {/* Row 6: code link */}
        {project.codeHref && (<a href={project.codeHref} target="_blank" rel="noopener noreferrer" onClick={function (e) { return e.stopPropagation(); }} className="mt-2.5 inline-flex items-center gap-1 font-mono-label text-[9px] uppercase tracking-[0.15em] text-ink-soft hover:text-ink transition-colors duration-200 self-start group">
            <span>Code</span>
      ...
      <dialog_1.DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid gap-5">
          ...
          <div className="rounded-3xl bg-paper p-5 border border-rule">
            ...
            <ul className="mt-3 space-y-3 text-sm leading-relaxed text-ink-soft">
              {project.engineering.map(function (item) { return (<li key={item} className="flex gap-3">
        ...
        <dialog_1.DialogFooter className="mt-6 gap-3">
          {project.codeHref ? (<a href={project.codeHref} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center rounded-full bg-ink px-5 py-3 text-sm font-semibold text-paper transition hover:bg-ink/90">
              View code
            </a>) : null}
...
/* ---------- 1. Fraud Detection ---------- */
function FraudArt(_a) {
    var stroke = _a.stroke, accent = _a.accent, alert = _a.alert;
    var sources = Array.from({ length: 6 }).map(function (_, i) { return ({ x: 30 + i * 18, y: 40 + (i % 3) * 14 }); });
    return (<g>
      {sources.map(function (s, i) { return (<g key={i}>
          <circle cx={s.x} cy={s.y} r="3" fill={accent} fillOpacity={i === 2 || i === 4 ? 0.8 : 0.25} stroke={accent} strokeWidth="0.6"/>
          <path d={"M ".concat(s.x, " ").concat(s.y, " C ").concat(s.x + 40, " ").concat(s.y, ", 130 130, 180 150")} stroke={i === 2 || i === 4 ? alert : stroke} strokeWidth={i === 2 || i === 4 ? 1.2 : 0.6} fill="none" strokeDasharray={i === 2 || i === 4 ? "4 3" : "none"} opacity={i === 2 || i === 4 ? 0.95 : 0.55}/>
        </g>); })}
...
function NlpArt(_a) {
    var stroke = _a.stroke, accent = _a.accent;
    return (<g>
      {[0, 1, 2].map(function (i) { return (<rect key={i} x={50 + i * 14} y={50 - i * 8} width="120" height="150" stroke={stroke} strokeWidth="0.5" fill="none" opacity={0.35 + i * 0.15}/>); })}
      <rect x="92" y="26" width="120" height="150" stroke={accent} strokeWidth="0.9" fill="#FFFFFF" fillOpacity="0.6"/>
      {Array.from({ length: 9 }).map(function (_, i) { return (<line key={i} x1={100} y1={38 + i * 14} x2={200 - (i === 4 ? 50 : i === 6 ? 30 : 0)} y2={38 + i * 14} stroke={i === 4 || i === 6 ? accent : stroke} strokeWidth={i === 4 || i === 6 ? 1.1 : 0.5} opacity={i === 4 || i === 6 ? 0.95 : 0.6}/>); })}
...
function EcommerceArt(_a) {
    ...
    return (<g stroke={stroke} strokeWidth="0.8" fill="none">
      ...
      {[0, 1, 2].map(function (i) { return (<circle key={i} cx={52 + i * 9} cy={51} r="2" fill={stroke} stroke="none" opacity="0.6"/>); })}
...
function PlaylistArt(_a) {
    ...
    return (<g stroke={stroke} strokeWidth="0.8" fill="none">
      <g>
        ...
        {Array.from({ length: 8 }).map(function (_, i) { return (<line key={i} x1={40 + (i % 4 === 3 ? 8 : 0)} y1={90 + i * 13} x2={100 - (i % 4 === 2 ? 6 : 0)} y2={90 + i * 13} stroke={stroke} strokeWidth="0.4" opacity="0.55"/>); })}
        {Array.from({ length: 8 }).map(function (_, i) { return (<line key={"r".concat(i)} x1={120} y1={90 + i * 13} x2={180 - (i % 3 === 1 ? 10 : 0)} y2={90 + i * 13} stroke={stroke} strokeWidth="0.4" opacity="0.55"/>); })}
      ...
      <g transform="translate(260, 110)">
        {Array.from({ length: 5 }).map(function (_, i) { return (<line key={i} x1="0" y1={i * 5} x2="120" y2={i * 5} stroke={accent} strokeWidth="0.5"/>); })}
        <path d="M 6 4 C 12 4, 12 14, 6 14 C 0 14, 0 4, 6 4 Z M 6 14 L 6 22" stroke={accent} strokeWidth="0.9" fill="none"/>
        {[{ x: 24, y: 12 }, { x: 40, y: 8 }, { x: 56, y: 14 }, { x: 72, y: 6 }, { x: 88, y: 12 }, { x: 104, y: 4 }].map(function (n, i) { return (<g key={i}><ellipse cx={n.x} cy={n.y} rx="2.5" ry="1.8" fill={accent} stroke="none" transform={"rotate(-20 ".concat(n.x, " ").concat(n.y, ")")}/><line x1={n.x + 2.3} y1={n.y - 0.5} x2={n.x + 2.3} y2={n.y - 9} stroke={accent} strokeWidth="0.7"/></g>); })}
...
/* ---------- 6. CryptoZombies ---------- */
function BlockchainArt(_a) {
    var stroke = _a.stroke, accent = _a.accent, alert = _a.alert;
    var blocks = [{ x: 40, y: 130 }, { x: 110, y: 80 }, { x: 180, y: 130 }, { x: 250, y: 80 }, { x: 320, y: 130 }];
    return (<g>
      {blocks.slice(0, -1).map(function (b, i) { return (<line key={i} x1={b.x + 18} y1={b.y + (i % 2 === 0 ? 10 : -10)} x2={blocks[i + 1].x - 18} y2={blocks[i + 1].y + (i % 2 === 0 ? -10 : 10)} stroke={accent} strokeWidth="1" strokeDasharray="3 2" opacity="0.7"/>); })}
      {blocks.map(function (b, i) {
            var isLatest = i === blocks.length - 1;
            return (<g key={i}>
            <polygon points={["".concat(b.x, ",").concat(b.y - 22), "".concat(b.x + 19, ",").concat(b.y - 11), "".concat(b.x + 19, ",").concat(b.y + 11), "".concat(b.x, ",").concat(b.y + 22), "".concat(b.x - 19, ",").concat(b.y + 11), "".concat(b.x - 19, ",").concat(b.y - 11)].join(" ")} stroke={isLatest ? alert : accent} strokeWidth={isLatest ? 1.3 : 1} fill={isLatest ? alert : accent} fillOpacity={isLatest ? 0.1 : 0.05}/>
            ...
            <g transform={"translate(".concat(b.x, ", ").concat(b.y - 12, ")")} opacity="0.85"><rect x="-4" y="-2" width="8" height="6" stroke={isLatest ? alert : accent} strokeWidth="0.5" fill="none"/><circle cx="-2" cy="0" r="0.7" fill={isLatest ? alert : accent} stroke="none"/><circle cx="2" cy="0" r="0.7" fill={isLatest ? alert : accent} stroke="none"/></g>
          </g>);
        })}
...
function DashboardArt(_a) {
    ...
    return (<g stroke={stroke} strokeWidth="0.8" fill="none">
      ...
      <g>
        {[60, 100, 140, 180].map(function (y) { return (<line key={y} x1="30" y1={y} x2="280" y2={y} stroke={stroke} strokeWidth="0.3" opacity="0.3" strokeDasharray="2 3"/>); })}
        {[{ y: 64, v: "+8%" }, { y: 104, v: "+4%" }, { y: 144, v: "0%" }, { y: 184, v: "-4%" }].map(function (row) { return (<text key={row.v} x="286" y={row.y + 2} fontFamily="ui-monospace, monospace" fontSize="5" fill={stroke} letterSpacing="0.3">{row.v}</text>); })}
        <line x1="30" y1="144" x2="280" y2="144" stroke={stroke} strokeWidth="0.5" opacity="0.5"/>
        {candles.map(function (c, i) { return (<g key={i}><line x1={c.x} y1={c.open - 8} x2={c.x} y2={c.close + 8} stroke={c.up ? accent : alert} strokeWidth="0.6"/><rect x={c.x - 4} y={Math.min(c.open, c.close)} width="8" height={Math.max(Math.abs(c.close - c.open), 2)} fill={c.up ? accent : alert} fillOpacity={c.up ? 0.7 : 0.6} stroke={c.up ? accent : alert} strokeWidth="0.5"/></g>); })}
      ...
      <g transform="translate(30, 240)">
        {["1D", "1W", "1M", "1Y", "ALL"].map(function (label, i) { return (<g key={label}><rect x={i * 38} y="0" width="32" height="18" stroke={i === 2 ? accent : stroke} strokeWidth="0.6" fill={i === 2 ? accent : "none"} fillOpacity={i === 2 ? 0.1 : 0} rx="2"/><text x={i * 38 + 16} y="11" textAnchor="middle" fontFamily="ui-monospace, monospace" fontSize="5.5" fill={i === 2 ? accent : stroke} letterSpacing="0.3" fontWeight={i === 2 ? "600" : "400"}>{label}</text></g>); })}
      </g>
    </g>);
}
/* ---------- Metric extractor ---------- */
function extractMetric(project) {
    for (var _i = 0, _a = project.engineering; _i < _a.length; _i++) {
        var note = _a[_i];
        var percentMatch = note.match(/(\d+%)/);
        if (percentMatch)
            return { value: percentMatch[1], caption: project.outcome.split(/[.]/)[0] };
    }
    for (var _b = 0, _c = project.engineering; _b < _c.length; _b++) {
        var note = _c[_b];
        var numMatch = note.match(/(\d+(?:,\d{3})?(?:\.\d+)?(?:K|M|\+)?)\b/);
        if (numMatch)
            return { value: numMatch[1], caption: project.outcome.split(/[.]/)[0] };
    }
    if (project.outcome)
        return { value: "•", caption: project.outcome.split(".")[0] };
    return null;
}
